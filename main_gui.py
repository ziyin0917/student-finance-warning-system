...
class Record:
    ...
    kind: str      # "收入" / "支出"
    note: str      # 自訂備註（可空）
    category: str  # 支出必選（飲食/交通/…）；收入固定 "收入"
    amount: float
...
class App(QWidget):
    ...
    def _build_tab_add(self):
        ...
        self.category.setCurrentText("飲食")  # ✅ 預設飲食（省點選）
    ...
    def _month_stats(self, m: str):
        ...
        return income, expense_total, dict(by_cat)

    def _refresh_add_chart(self):
        m = self._current_month()
        income, expense_total, by_cat = self._month_stats(m)
        plot_monthly(by_cat, income, expense_total, self.canvas_add, m, note="（記帳頁）")
    ...
    def _refresh_report(self):
        ...
        plot_monthly(by_cat, income, expense_total, self.canvas_report, m, note="（月報）")
